...
class FunArgsTest(TestCase):
    ...
    def testVariables(self):
        ...
        result = self.render("{{ f|arg:a|kw:k|arg:v }}",
                             f=TF(f), a=2, k='y', v=3)
        self.assertEqual(result, '5')
    ...
    def testUnpackArgs(self):
        ...
        result = self.render("{{ f|unpack_args:xs }}", f=TF(f), xs=xs)
    ...
    def testUnpackKwargs(self):
        ...
        result = self.render("{{ f|unpack_args:ms }}", f=TF(f), ms=ms)
    ...
    def testMethod(self):
        ...
        result = self.render("{{ obj|method:'get'|arg:'bar' }}", obj=obj)
        self.assertEqual(result, 'foobar')

    def testStringFormat(self):
        s = '{x}{y}'
        m = dict(x='foo', y='bar')
        self.assertEqual(s.format(**m), 'foobar')
        result = self.render("{{ s|method:'format'|unpack_args:m }}",
                             s=s, m=m)
        self.assertEqual(result, 'foobar')
